...
const DIRECTIONS = [
  ...
  },
  {
    title: '현장 중심 교육과정 운영',
    items: [
      '발전소 견학 프로그램 확대 실시 (원자력계통 → 원자력기초)',
      '각 교육과정 현장교육 확대 시행',
    ],
    color: '#0072CE',
  },
  {
    title: '홍보활동 강화 및 수요조사 다각화',
    items: [
      '다양한 채널을 통한 홍보활동 강화',
  ...
  return (
    <div className="slide">
      ...
      <div className="relative z-10" style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'clamp(14px, 1.4vw, 24px)' }}>
          {DIRECTIONS.map((d, i) => (
            <div key={i} className={`glass-card entrance entrance-d${i + 2}`} style={{
              padding: 'clamp(22px, 2.2vw, 40px)',
              borderLeft: `4px solid ${d.color}`,
            }}>
              <div style={{ position: 'relative', zIndex: 1, display: 'flex', gap: 'clamp(16px, 1.8vw, 32px)', alignItems: 'flex-start' }}>
                {/* Number */}
                <div className="float-y" style={{
                  width: 'clamp(50px, 4vw, 72px)', height: 'clamp(50px, 4vw, 72px)',
                  borderRadius: '50%', flexShrink: 0,
                  background: `${d.color}18`,
                  border: `2px solid ${d.color}35`,
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  boxShadow: `0 0 24px ${d.color}20`,
                  animationDelay: `${i * 0.5}s`,
                }}>
                  <span style={{ fontWeight: 900, fontSize: 'clamp(20px, 1.8vw, 32px)', color: d.color }}>
                ...
                <div style={{ flex: 1 }}>
                  <h3 style={{
                    color: '#fff', marginBottom: 'clamp(8px, 0.7vw, 14px)',
                    fontSize: 'clamp(24px, 2.2vw, 40px)', fontWeight: 700,
                  }}>
                    {d.title}
                  </h3>
                  <ul style={{ listStyle: 'none', padding: 0 }}>
                    {d.items.map((item, j) => (
                      <li key={j} style={{
                        fontSize: 'clamp(16px, 1.4vw, 26px)', color: 'rgba(255,255,255,0.65)',
        ...
        {/* Vision statement */}
        <div className="entrance entrance-d6" style={{ marginTop: 'clamp(24px, 2.5vw, 44px)', textAlign: 'center' }}>
          <div style={{
            display: 'inline-block',
            padding: 'clamp(20px, 2vw, 36px) clamp(40px, 4.5vw, 80px)',
            borderRadius: 20,
